...
- Login and signup attempts were not limited by account identifier, so account-focused guessing was harder to detect.
...
- Session and environment security checks were weak, and authenticated sessions were not renewed after login/signup.
...
| Rate limiting | Added route-level limits for login and signup by IP and normalized account/email identifier; stores counters in MongoDB by default; returns `429` with `Retry-After` when the limit is exceeded. | `src/middleware/rateLimit.js`, `src/models/rateLimit.model.js`, `src/routes/login.route.js`, `src/routes/signup.route.js` |
...
| Session and account hardening | Hardened session cookie options; renewed the session after successful login/signup to reduce session fixation risk; added startup checks for missing or weak secrets; required password re-authentication before account deletion. | `src/utilities/config.js`, `src/utilities/session.js`, `src/app.js`, `views/partials/dashboard/navbar-dashboard.ejs` |
...
The implementation strengthened account actions such as signup, login, logout, and account deletion by adding CSRF protection, IP and account-aware login/signup rate limiting, centralized input validation, session renewal after authentication, hardened session and account-deletion controls, security headers, centralized error handling, and privacy-aware audit logging. These changes should be presented together as one security deficiency fix, with concise before/after code snippets selected from the most important files.
